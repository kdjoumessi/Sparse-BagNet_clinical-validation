...
def train(cfg, model, train_dataset, val_dataset, estimator, logger=None):
    ...
    for epoch in range(1, cfg.train.epochs + 1):
        ...
        for step, train_data in progress:
            ...
            # forward
            y_pred, x_local, x_weight = model(X)
        ...
        '''
        if cfg.data.binary:
            message = '{} => epoch {}/{} Training accuracy: {}, auc: {}, auprc: {}'
            progress.set_description(message.format('Training', epoch, cfg.train.epochs, acc, t_auc, t_auprc))
        ...
        if epoch % cfg.train.eval_interval == 0:
            ...
            v_acc, v_bin_acc = estimator.get_accuracy(6)
            ...
            if cfg.data.binary:                
                print('{} => epoch {}/{} Trainin accuracy: {}, auc: {}, auprc: {}'.format(dtime, epoch, cfg.train.epochs, avg_acc, t_auc, t_auprc))
                progress.set_description('{} => epoch {}/{} validation accuracy: {}, auc: {}, auprc: {}'.format('Validation', epoch, cfg.train.epochs, v_acc, v_auc, v_auprc))
                print('{} => epoch {}/{} validation accuracy: {}, auc: {}, auprc: {}'.format(dtime, epoch, cfg.train.epochs, v_acc, v_auc, v_auprc))
            else:
                print('{}: epoch {}/{} Taining accuracy: {}, kappa: {}'.format(dtime, epoch, cfg.train.epochs, avg_acc, avg_kappa))
                progress.set_description('{} => epoch {}/{} validation accuracy: {}, kappa: {}'.format('Validation', epoch, cfg.train.epochs, v_acc, v_kappa))
                print('{}: epoch {}/{} validation accuracy: {}, kappa: {}'.format(dtime, epoch, cfg.train.epochs, v_acc, v_kappa))
            
            if logger:
                logger.add_scalar('validation loss', loss_, epoch)
                logger.add_scalar('validation accuracy', v_acc, epoch)
                ...
                if cfg.data.binary:
                    ...
                    if cfg.data.num_classes != 2:
                        logger.add_scalar('validation bin accuracy', v_bin_acc, epoch)
                        val_bin_acc.append(v_bin_acc)                    
            ...
            val_loss.append(loss_); val_acc.append(v_acc)             

            # save model
            (indicator, bin_indicator) = (v_kappa, None) if cfg.train.kappa_prior else (v_acc, v_bin_acc)
